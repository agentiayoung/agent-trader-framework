...
> interactif de the maintainer (`<EMAIL>`) et heurtaient les blocages de quota
> (« You're out of extra usage »). On les isole sur un **compte Max dédié (Teodore)** sans toucher
> la session interactive de the maintainer.
...
vers `<USER_HOME>\.claude-teodore` **UNIQUEMENT si ses credentials existent** (sinon fallback sur le
compte courant → zéro casse tant que le login n'est pas fait). Override : `$env:ROUTINE_CLAUDE_CONFIG_DIR`.
La session interactive de the maintainer (`claude` normal) reste sur `~/.claude` / son compte — **non affectée**.
...
$env:CLAUDE_CONFIG_DIR = "<USER_HOME>\.claude-teodore"
...
claude /status   # ou: type <USER_HOME>\.claude-teodore\.credentials.json existe
```

Après ça, `<USER_HOME>\.claude-teodore\.credentials.json` existe → **les routines basculent
...
- Dans `routines\logs\routine_*.log`, la 1re ligne doit être `[COMPTE] routine sur CLAUDE_CONFIG_DIR=<USER_HOME>\.claude-teodore (compte dedie Max)`.
...
- Pour révoquer : supprimer `<USER_HOME>\.claude-teodore\.credentials.json` (routines reviennent au compte par défaut).
